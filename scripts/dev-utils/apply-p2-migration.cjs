...
async function run() {
  console.log('▶️ Aplicando migración P2 (20260916_p2_advisory_lock_ai_worker.sql)...');
  const migPath = path.resolve(__dirname, '../../supabase/migrations/20260916_p2_advisory_lock_ai_worker.sql');
  ...
  if (!res.ok) {
    console.error('❌ Error aplicando migración P2:', body);
    process.exit(1);
  }

  console.log('✅ Migración P2 aplicada exitosamente.');
