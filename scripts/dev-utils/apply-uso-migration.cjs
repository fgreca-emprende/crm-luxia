...
async function run() {
  console.log('▶️ Aplicando migración usuario_uso_diario RLS...');
  const migPath = path.resolve(__dirname, '../../supabase/migrations/20260917_p0_fix_usuario_uso_diario_rls.sql');
  ...
  if (!res.ok) {
    console.error('❌ Error aplicando migración:', body);
    process.exit(1);
  }
